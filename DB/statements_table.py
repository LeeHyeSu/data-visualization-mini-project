...
for col in fin_df.columns[2:]:
    fin_df[col] = fin_df[col].apply(lambda x: x.replace(',',''))
    fin_df[col] = pd.to_numeric(fin_df[col])
...
db=pymysql.connect(host='localhost',port=3303,user='root', passwd='1208',db='finance')
...
engine = create_engine("mysql://root:pass@127.0.0.1/finance",encoding="utf-8")
fin_df.to_sql(name="statements",con=engine,if_exists="append",index=False)
